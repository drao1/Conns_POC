...
public class TestNGCustomReportListener implements IReporter {
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
			    ...
			    calendar.setTimeInMillis(startMS);
			     String value1=formatter.format(calendar.getTime());
			     calendar.setTimeInMillis(end*1000);
			     String value2=formatter.format(calendar.getTime());
				mq += 1;
				if (mq > 1) {
	...
	protected void writeSummary(String a) {
		writer.print("<hr><hr-line></hr>");
		writer.print("<h2><h2-title>" + a+"</h2>");
	}
	/*
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println(".passedodd td {background-color: #dff0d8}");
		out.println(".passedeven td {background-color: #dff0d8}");
		
		out.println(".skippedodd td {background-color: #fff3c4}");
		out.println(".skippedodd td {background-color: #fff3c4}");
		
		out.println(".failedodd td,.numi_attn {background-color: #f2dede}");
		out.println(".failedeven td,.stripe .numi_attn {background-color: #f2dede}");

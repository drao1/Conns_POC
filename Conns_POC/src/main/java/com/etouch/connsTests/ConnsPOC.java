...
public class ConnsPOC extends BaseTest {
	...
	static String videoLocation = AbsolutePath
			.substring(0, AbsolutePath.indexOf("/target/classes/"))
			.substring(1).concat("/src/test/resources/testdata/videos");
	...
	public void setUp(ITestContext context) throws InterruptedException,
		...
		try {
			...
			browserName = TestBedManager.INSTANCE.getCurrentTestBeds()
					.get(testBedName).getBrowser().getName();
			...
			try {
				...
				commonData = ExcelUtil.readExcelData(DataFilePath, "CreditApp",
						"CreditAppCommonElements");

				checkoutFlowCommonLocators = ExcelUtil.readExcelData(
						DataFilePath, "ProductPurchase",
						"Checkout_Flow_Common_Locators");
				System.out.println("checkoutFlowCommonLocators[33][4] : "
						+ checkoutFlowCommonLocators[33][4]);
				frenchDoor = ExcelUtil.readExcelData(DataFilePath,
						"ProductPurchase", "Click_On_French_Door");
				mobileMenuData = ExcelUtil.readExcelData(DataFilePath,
						"ProductPurchase", "Mobile_Menu_Details");
				proceedToCheckout = ExcelUtil.readExcelData(DataFilePath,
						"ProductPurchase", "Proceed_To_Checkout_Button");
				checkoutGuest = ExcelUtil.readExcelData(DataFilePath,
						"ProductPurchase", "Checkout_Guest");
				submitBillingInfo = ExcelUtil.readExcelData(DataFilePath,
						"ProductPurchase", "Submit_Billing_Information");
				...
				if (testType.equalsIgnoreCase("Web")) {
					System.out.println("videoLocation"
							+ videoLocation.toString().replace("Env", testEnv));
				}
...
//					try {
//						if (!TestBedManager.INSTANCE.getCurrentTestBeds()
//								.get(testBedName).getBrowser().getWidth()
//								.isEmpty()
//								&& !TestBedManager.INSTANCE
//										.getCurrentTestBeds().get(testBedName)
//										.getBrowser().getHeight().isEmpty()) {
//
//							int width = Integer
//									.parseInt(TestBedManager.INSTANCE
//											.getCurrentTestBeds()
//											.get(testBedName).getBrowser()
//											.getWidth());
//							int height = Integer
//									.parseInt(TestBedManager.INSTANCE
//											.getCurrentTestBeds()
//											.get(testBedName).getBrowser()
//											.getHeight());
//							log.info("Setting Window resolution to : " + width
//									+ " X " + height);
...
//					}
	...
	public void Verify_ChoseYourRegion_Links() throws PageException,
		...
		try {
			...
			String[][] regionLinksData = ExcelUtil.readExcelData(DataFilePath,
					"StoreLocator", "verifyChoseYourRegionLinks");
			for (int i = 0; i < regionLinksData.length; i++) {
				commonMethods.navigateToPage(webPage, storeLocatorURL,
						softAssert);
				connsStoreLocatorPage.closeLocationPopup(webPage, softAssert);
				if (testType.equalsIgnoreCase("Web")) {
					connsStoreLocatorPage.closeLocationPopup(webPage,
							softAssert);
					commonMethods.hoverOnelementbyXpath(webPage,
							regionLinksData[i][1], softAssert);
					String afterLinkHover = commonMethods
							.getCssvaluebyXpath(webPage, regionLinksData[i][1],
									"color", softAssert);
					log.info("Region " + regionLinksData[i][0]
							+ " after hover color attribute value is : "
							+ afterLinkHover);
					if (!regionLinksData[i][3].equalsIgnoreCase("NA")) {
						softAssert.assertEquals(afterLinkHover,
								regionLinksData[i][3],
								"Hover functionality failed for link "
										+ regionLinksData[i][0]
										+ " Expected color: "
										+ regionLinksData[i][3]
										+ " Actual color: " + afterLinkHover);
					}
				}
				String actualUrl = commonMethods.clickAndGetPageURL(webPage,
						regionLinksData[i][1], regionLinksData[i][0],
						softAssert, commonData[2][1]);
				softAssert.assertEquals(actualUrl, regionLinksData[i][2],
						"URL verification failed for link : '"
								+ regionLinksData[i][0] + "'. Expected URL - "
								+ regionLinksData[i][2] + " Actual URL - "
								+ actualUrl);
			}
			softAssert.assertAll();
		} catch (Throwable e) {
			mainPage.getScreenShotForFailure(webPage,
					"Verify_ChoseYourRegion_Links");
			softAssert.assertAll();
		...
		try {
			...
			if (testType.equalsIgnoreCase("Web")) {
				connsProductPurchasePage.Click_On_Element_JS(webPage,
						frenchDoor[1][1], softAssert);
				commonMethods.selectDropdownByValue(webPage,
						checkoutFlowCommonLocators[17][1], "28", softAssert);
			} else {
				connsProductPurchasePage.clickOnMobileMenuOption(webPage,
						mobileMenuData, softAssert);
			}
			// adding pickup only product to cart
			productDetails = connsProductPurchasePage
					.Click_On_In_Stock_With_Delivery_Available(webPage,
							pickupAvialableProduct, softAssert);
			List<String> actualValue = connsProductPurchasePage
					.page_Verify_Product_Details_Cart(webPage,
			...
			softAssert
					.assertTrue(productDetails.contains(actualValue.get(0)),
			...
			softAssert.assertTrue(productDetails.contains(actualValue.get(1)),
					"Cart price product is not matching with list price:"
							+ "excpected is:" + productPriceCartPage);

			connsProductPurchasePage.Proceed_To_Checkout_Button(webPage,
					proceedToCheckout, softAssert);
			connsProductPurchasePage.Checkout_Guest(webPage, checkoutGuest,
					softAssert);
			connsProductPurchasePage.Submit_Billing_Information(webPage,
					submitBillingInfo, softAssert);
			connsProductPurchasePage.click_Billing_Info_Continue_Button(
					webPage, checkoutFlowCommonLocators, softAssert);
			CommonMethods.waitForWebElement(
					By.xpath(checkoutFlowCommonLocators[6][1]), webPage);

			// commonMethods.scrollToElement(checkoutFlowCommonLocators[6][1],
			// webPage,softAssert);

			String productNamePickupLocationSection = connsProductPurchasePage
					.get_Pickup_Location_Product_Name(webPage,
							checkoutFlowCommonLocators, softAssert);
			softAssert
					.assertTrue(
							productDetails
									.contains(productNamePickupLocationSection),
							"product name displayed in pickup location section is not matching is not matching with list page "
									+ "expected is:"
									+ productNamePickupLocationSection);
			CommonMethods.waitForWebElement(
					By.xpath(checkoutFlowCommonLocators[5][1]), webPage);

			connsProductPurchasePage.click_Pickup_Location_Continue_Button(
					webPage, checkoutFlowCommonLocators, softAssert);
			...
			// checkoutFlowCommonLocators, softAssert);
			CommonMethods.waitForWebElement(
					By.xpath(checkoutFlowCommonLocators[9][1]), webPage);
			connsProductPurchasePage.click_Cash_On_Delivery_Radio_Button(
					webPage, checkoutFlowCommonLocators, softAssert);
			connsProductPurchasePage.click_Payment_Info_Continue_Button(
					webPage, checkoutFlowCommonLocators, softAssert);
			...
			CommonMethods.waitForWebElement(
					By.xpath(checkoutFlowCommonLocators[15][1]), webPage);
			connsProductPurchasePage.click_Place_Order_Button(webPage,
					checkoutFlowCommonLocators, softAssert);

			String orderConfirmationText = commonMethods.getTextbyXpath(
					webPage, checkoutFlowCommonLocators[33][1], softAssert);

			softAssert.assertEquals(orderConfirmationText,
					expectedSuccessMessage,
					"order confirmation message is not matching:"
							+ " expected is:" + expectedSuccessMessage
			...
			mainPage.getScreenShotForFailure(webPage,
					"Verify_Checkout_Flow_Cash_On_Delivery");
			SoftAssertor.addVerificationFailure(e.getMessage());
